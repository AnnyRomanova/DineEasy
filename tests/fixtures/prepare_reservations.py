...
def reservations(tables: list) -> list[Reservation]:
    base_date = datetime(2025, 12, 1, 12, 0, tzinfo=timezone.utc)
    reservations = []

    for i in range(5):
        random_days = random.randint(0, 30)
        random_minutes = random.randint(0, 720)
        reservation_time = base_date + timedelta(
...
async def prepare_reservations(
    dine_easy_db: DatabaseConnector, prepare_tables, reservations: list[Reservation]
) -> list[Reservation]:
